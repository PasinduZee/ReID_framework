function result=KernelXCRC_VIPER(c2o_params,params,features)
...
rng(10);
    ...
    %let data A is the probe and data B is the gallery
    feat.dataA = dt(c2o_params.idxtrain,:); 
    feat.dataB = dt(c2o_params.idxtrain + params.N,:); 
    
    %set probe image
    if(c2o_params.probe_cam == 1)
         test_feat.dataA = dt(c2o_params.idxtest_probe + params.N,:);
    else
         test_feat.dataA = dt(c2o_params.idxtest_probe,:);
    end
    
    test_feat.dataB=[];
    for i=1:numel(c2o_params.idxtest_gallery)       
       index=find(c2o_params.idxtest_gallery==c2o_params.idxtest_gallery(i));
       if(index(1) ~= i)
           test_feat.dataB=[test_feat.dataB ; dt(c2o_params.idxtest_gallery(i),:)];
       else
           test_feat.dataB=[test_feat.dataB ; dt(c2o_params.idxtest_gallery(i) + params.N,:)];
       end
    end    
    ...
    result=idx_error(1,1:numel(c2o_params.idxtest_gallery));
