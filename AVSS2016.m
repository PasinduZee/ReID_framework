function result =AVSS2016(c2o_params,params)
...
load(sprintf('./auxiliary/%s_lomo_mix_avss2016.mat',c2o_params.dataset));

params.ModeAfactors = 125; 
params.XKPLSfactors = 100;

    % camera b is the probe and cam a is the gallery
    feat.dataA = Xview1(:,c2o_params.idxtrain)'; 
    feat.dataB = Xview2(:,c2o_params.idxtrain)';
    
    test_feat.dataA = Xview1(:,c2o_params.idxtest_gallery)';
    
    %set probe image
    if(c2o_params.probe_cam == 1)
         test_feat.dataB = Xview1(:,c2o_params.idxtest_probe)';
    else
         test_feat.dataB = Xview2(:,c2o_params.idxtest_probe)';
    end

   % Applying Kernel 
   % optimization with mat files
    mat_name=strcat('./auxiliary/','kernel_values_AVSS_',num2str(c2o_params.iter_id),'.mat');
    ...
    if exist(mat_name)
        ...
        [test_a_ker_random] = kernel_expchi2(test_feat.dataA(1:c2o_params.two_folder_point,:),feat.dataA,omegaA);
        save(mat_name,'train_a_ker','omegaA','omegaB','train_b_ker','test_a_ker_random');
    end
    
    [test_a_ker_known] = kernel_expchi2(test_feat.dataA(c2o_params.two_folder_point+1:numel(c2o_params.idxtest_gallery),:),feat.dataA,omegaA);
    [test_a_ker] = [test_a_ker_random ; test_a_ker_known];
    ...
    result = run_XKPLS(kerneldata,XKPLSparams);  

end

function cmc_iter = run_kpls(norm_test_a_ker,norm_test_b_ker, PLSparams,iter,resp_final_kpls)
    ...
    for i=1:num_test
        ...
        resp_final_kpls(iter,idx(i,:)==i)= resp_final_kpls(iter,idx(i,:)==i)+1;
    end
    cmc_iter = cumsum(nresp)./num_test;
end 

function cmc_iter = run_plsOAA(test_featA, test_featB, PLSparams,iter,resp_final_plsOAA)
    ...
    for i=1: num_test
        nresp(idx(i,:)==i) = nresp(idx(i,:)==i) + 1;
        resp_final_plsOAA(iter,idx(i,:)==i)= resp_final_plsOAA(iter,idx(i,:)==i)+1;
    end
    cmc_iter = cumsum(nresp)./num_test;
end

function cmc_iter = run_kpls2modeA(data,PLSparams,iter,resp_final_kplsMode)
    %  NIPALS KPLS mode A
    tic
...
function result = run_XKPLS(data,PLSparams)
    ...
    YstarA = data.norm_test_a_ker*ModelA.W; 
    YstarA = normr(YstarA);
    YstarB = data.norm_test_b_ker*ModelB.W; 
    YstarB = normr(YstarB);

    resp = pdist2(YstarA,YstarB,'cosine')'; 
    [~,idx]=sort(resp,2,'ascend');
    result=idx;
end
