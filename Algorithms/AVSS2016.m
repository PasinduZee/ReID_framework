...
resp_final_kplsMode=zeros(params.Iter,numel(trials(1).labelsAtest));
resp_final_xkpls=zeros(params.Iter,numel(trials(1).labelsAtest));
resp_final_plsOAA=zeros(params.Iter,numel(trials(1).labelsAtest));
resp_final_kpls=zeros(params.Iter,numel(trials(1).labelsAtest));
...
for iter=1:params.Iter
   ...
   [test_b_ker] = kernel_expchi2(test_feat.dataB,feat.dataB,omegaB);
    ...
    kplsMode(iter,:) =  run_kpls2modeA(kerneldata,KPLSModeA_params,iter,resp_final_kplsMode);
    ...
    xkpls(iter,:) = run_XKPLS(kerneldata,XKPLSparams,iter,resp_final_xkpls);  
    ...
    plsOAA(iter,:) = run_plsOAA(test_feat.dataA,test_feat.dataB,PLSparams,iter,resp_final_plsOAA);
...
%     
    kpls(iter,:) = run_kpls(test_a_ker,test_b_ker,KPLSparams,iter,resp_final_kpls);
end
...
function cmc_iter = run_kpls2modeA(data,PLSparams,iter,resp_final_kplsMode)
    ...
    for i=1:num_test
        ...
        resp_final_kplsMode(iter,rank(i,:)==i)= resp_final_kplsMode(iter,rank(i,:)==i)+1;
    end
    
    cmc_iter = cumsum(responsePLS)./num_test;
end

function cmc_iter = run_XKPLS(data,PLSparams,iter,resp_final_xkpls)
    ...
    for i=1:num_test
        nresp(idx(i,:)==i) = nresp(idx(i,:)==i) + 1;
        resp_final_xkpls(iter,idx(i,:)==i)= resp_final_xkpls(iter,idx(i,:)==i)+1;
    end
    cmc_iter = cumsum(nresp)./num_test;
end
